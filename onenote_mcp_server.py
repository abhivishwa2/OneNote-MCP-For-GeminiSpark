...
async def ensure_valid_token() -> bool:
    ...
    global access_token, msal_app, refresh_token
    
    # 1. First, try loading cached tokens (if they exist)
    if not access_token:
        load_tokens()
    
    # 2. Check if current in-memory token is still valid
    if access_token and token_expires_at and time.time() < token_expires_at:
        return True
    
    # 3. HEADLESS BYPASS: Inject Cloud Run Environment Variable
    env_refresh = os.getenv("AZURE_REFRESH_TOKEN")
    if env_refresh:
        logger.info("Headless Mode: Using AZURE_REFRESH_TOKEN from environment variables.")
        refresh_token = env_refresh  # Override the global refresh token
    
    # 4. Try to refresh the token using the injected refresh_token
    if not msal_app:
...
async def make_graph_request(endpoint: str, method: str = "GET", data: Dict = None) -> Dict:
    ...
    # HEADLESS AUTH: Trigger background token refresh automatically
    if not await ensure_valid_token():
        raise Exception("Authentication failed: Server could not silently refresh the token. Ensure AZURE_REFRESH_TOKEN is set in Google Cloud Run.")
...
def main():
 ...
 # --- GEMINI VALIDATOR FINAL PATCH ---
    original_call = starlette.applications.Starlette.__call__
    
    async def validator_bypass(self, scope, receive, send):
        global access_token 
        
        if scope["type"] == "http":
            # 1. Force HTTPS and Host for Cloud Run routing
            scope["scheme"] = "https"
            headers_dict = dict(scope.get("headers", []))
            if b"x-forwarded-host" in headers_dict:
                new_headers = [(k, v) for k, v in scope["headers"] if k != b"host"]
                new_headers.append((b"host", headers_dict[b"x-forwarded-host"]))
                scope["headers"] = new_headers

            # 2. EXTRACT SPARK'S OAUTH TOKEN
        ...
        async def custom_send(message):
            if message["type"] == "http.response.start":
                ...
                headers = [(k, v) for k, v in headers if k.lower() not in (b"access-control-allow-origin", b"access-control-allow-methods", b"access-control-allow-headers")]
        ...
        if scope["type"] == "http":
            if scope["method"] == "OPTIONS":
                await custom_send({"type": "http.response.start", "status": 200})
                await custom_send({"type": "http.response.body", "body": b""})
                return

            # INTERCEPT ALL POST REQUESTS (Bypassing FastMCP Session Validation)
            if scope["method"] == "POST" and scope["path"] in ("/sse", "/sse/", "/messages", "/messages/"):
                body = b""
                more_body = True
                while more_body:
                ...
                if method == "initialize":
                    ...
                    response_payload = {
                        ...
                        "result": {
                            ...
                            "capabilities": {"tools": {}}, 
                            "serverInfo": {"name": "onenote-mcp", "version": "1.0.0"}
                        }
                    }
                    await custom_send({"type": "http.response.body", "body": json.dumps(response_payload).encode('utf-8')})
                    return
                ...
                if method == "tools/list":
                    await custom_send({"type": "http.response.start", "status": 200, "headers": [(b"content-type", b"application/json")]})
                    tools = [
                        {"name": "list_notebooks", "description": "List all OneNote notebooks.", "inputSchema": {"type": "object", "properties": {}}},
                        {"name": "list_sections", "description": "List sections in a specific notebook.", "inputSchema": {"type": "object", "properties": {"notebook_id": {"type": "string"}}, "required": ["notebook_id"]}},
                        {"name": "list_pages", "description": "List pages in a specific section.", "inputSchema": {"type": "object", "properties": {"section_id": {"type": "string"}}, "required": ["section_id"]}},
                        {"name": "get_page_content", "description": "Get the content of a specific page.", "inputSchema": {"type": "object", "properties": {"page_id": {"type": "string"}}, "required": ["page_id"]}},
                        {"name": "create_notebook", "description": "Create a new OneNote notebook.", "inputSchema": {"type": "object", "properties": {"name": {"type": "string"}, "description": {"type": "string"}}, "required": ["name"]}},
                        {"name": "create_section", "description": "Create a new section in a OneNote notebook.", "inputSchema": {"type": "object", "properties": {"notebook_id": {"type": "string"}, "name": {"type": "string"}}, "required": ["notebook_id", "name"]}},
                        {"name": "create_page", "description": "Create a new page in a OneNote section.", "inputSchema": {"type": "object", "properties": {"section_id": {"type": "string"}, "title": {"type": "string"}, "content_html": {"type": "string"}}, "required": ["section_id", "title"]}},
                        {"name": "update_page_content", "description": "Update the content of an existing OneNote page.", "inputSchema": {"type": "object", "properties": {"page_id": {"type": "string"}, "content_html": {"type": "string"}, "target_element": {"type": "string"}}, "required": ["page_id", "content_html"]}}
                    ]
                ...
                # DIRECT TOOL EXECUTION (Bypasses FastMCP Engine)
                if method == "tools/call":
                    ...
                    try:
                        result_str = ""
                        # FIX: Added .fn to access the underlying function inside the FunctionTool object
                        if tool_name == "list_notebooks":
                            result_str = await list_notebooks.fn()
                        elif tool_name == "list_sections":
                            result_str = await list_sections.fn(args.get("notebook_id"))
                        elif tool_name == "list_pages":
                            result_str = await list_pages.fn(args.get("section_id"))
                        elif tool_name == "get_page_content":
                            result_str = await get_page_content.fn(args.get("page_id"))
                        elif tool_name == "create_notebook":
                            result_str = await create_notebook.fn(args.get("name"), args.get("description"))
                        elif tool_name == "create_section":
                            result_str = await create_section.fn(args.get("notebook_id"), args.get("name"))
                        elif tool_name == "create_page":
                            result_str = await create_page.fn(args.get("section_id"), args.get("title"), args.get("content_html"))
                        elif tool_name == "update_page_content":
                            target = args.get("target_element", "body")
                            result_str = await update_page_content.fn(args.get("page_id"), args.get("content_html"), target)
                        else:
                        ...
                        response_payload = {
                            ...
                            "id": req_id, 
                            "result": {
                                "content": [{"type": "text", "text": str(result_str)}],
                                "isError": False
                            }
                        }
                    except Exception as e:
                ...
                # Catch-all for unrecognized POST methods to prevent disconnects
                await custom_send({"type": "http.response.start", "status": 200, "headers": [(b"content-type", b"application/json")]})
                await custom_send({"type": "http.response.body", "body": b"{}"})
                return

            # OAuth discovery endpoint
            if scope["method"] == "GET" and (scope["path"].startswith("/.well-known/oauth-protected-resource") or scope["path"].startswith("/.well-known/oauth-authorization-server")):
                await custom_send({"type": "http.response.start", "status": 200, "headers": [(b"content-type", b"application/json")]})
                oauth_metadata = {
                    "authorization_server": "https://login.microsoftonline.com/consumers/v2.0",
                    "authorization_endpoint": "https://login.microsoftonline.com/consumers/oauth2/v2.0/authorize",
                    "token_endpoint": "https://login.microsoftonline.com/consumers/oauth2/v2.0/token"
                }
                await custom_send({"type": "http.response.body", "body": json.dumps(oauth_metadata).encode('utf-8')})
                return
            
        await original_call(self, scope, receive, send)
        
    starlette.applications.Starlette.__call__ = validator_bypass
    # ---------------------------------------------
